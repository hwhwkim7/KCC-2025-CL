...
int main(int argc, char** argv)
{
    ...
    budget=atoi(argv[2]);
    sprintf(input,"../dataset/%s/network_re.dat",argv[1]);
    sprintf(output,"../output/GCC/%s_%d.txt",argv[1],budget);
    ...
    {
        printf("%d\t%d\t%d\t%d\t",i,ans+Max_follower, Max_follower_pos, candidates);
        ans+=Max_follower;
        candidates = 0;
